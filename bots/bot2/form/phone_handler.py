...
import re
from form.message_handler import get_message_content
...
def register_phone_handlers(bot):
    ...
    async def callback(call):
        ...
        await bot.set_state(user_id, FormStates.phone, chat_id)
    ...
    async def form_phone(message):
        chat_id = message.chat.id
        user_id = message.from_user.id

        if re.match(r'^\+?[1-9][0-9]{7,14}$', message.text) is None:
            await bot.send_message(chat_id, 'Некорректный номер телефона, повторите попытку', parse_mode='markdown')
            return

        async with bot.retrieve_data(user_id, chat_id) as data:
            data['all_form_data']['phone'] = message.text

        await bot.set_state(user_id, FormStates.message, chat_id)
